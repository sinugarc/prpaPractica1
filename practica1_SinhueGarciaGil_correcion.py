...
def prod(common,tid,cap,limit,sem_empty,sem_nonempty,sec_critica):
    valor=0
...
def merge(common_list,res, indices, cap, sem_empty, sem_nonempty,sec_critica):
...
def main():
    ...
    lp.append(Process(target=merge, args=(common, res, indices,capacidades,semaforos_empty,
                                          semaforos_nonempty,sec_critica))
    for p in lp:
        p.start()
    for p in lp:
        p.join()
